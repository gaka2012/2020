...
if __name__ == '__main__':
    ...
    #print (data)
    '''
    img = cv2.imread('3.png',0)  #图片中的数据分为0和大于0的值，0部分是黑色，其他数值表现大约为白色，不知道为啥不直接弄成0和1
    #print (img)
    #img = cv2.adaptiveThreshold(img,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,117,8)
    ret, thresh_img = cv2.threshold(img,1,255,cv2.THRESH_BINARY) # 二值化
    print (thresh_img)
    cv2.imwrite('test.png',thresh_img)
    '''
    img = cv2.imread('333.png',0)
    ret, thresh_img = cv2.threshold(img,1,255,cv2.THRESH_BINARY_INV) # 二值化
    cv2.imwrite('3333.png',thresh_img)
