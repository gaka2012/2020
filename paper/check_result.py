...
#7.1读取准备删除的图片，获取其文件名称，然后删除相应的数据。
'''
data_path  = '/home/zhangzhipeng/software/data/fp_data/waste' #遍历准备删除的图片 
save_dir = '/home/zhangzhipeng/software/github/data/figure'  #将sac三分量画图后保存位置。

data_files = sorted(glob.glob(data_path+'/*.png'))       

data_list = []
for data in data_files:
    #将每一个图片的名字替换为BHZ.sac，存储在列表中，准备删除
    file_name   = os.path.basename(data)
    figure_name = file_name.replace('png','BHZ.sac')
    data_list.append(figure_name)
    
print (len(data_list))

with open('png.json','w') as file_obj:
    json.dump(data_list,file_obj)
'''

'''
data_path  = '/bashi_fs/centos_data/zzp/120_data' #遍历原始数据所在位置，删除列表中的数据


filename='png.json'
with open(filename) as file_obj:
    png = json.load(file_obj)

print (png[:4])
wrong_num = 0
sac_datas = glob.glob(data_path+'/*.BHZ.sac')     
total = len(sac_datas)
os.chdir(data_path)

for sacs in sac_datas:
    sac_name = os.path.basename(sacs)
    if sac_name  in png:
        wrong_num +=1
        #os.system('rm %s '%(sac_name))        
        
print ('there are %s datas and %s of them have been moved'%(total,wrong_num))
