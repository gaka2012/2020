...
class Network(object):
    ...
    def __init__(self, sizes):
        ...
        self.sizes = sizes                     
        self.biases = [np.random.randn(y, 1) for y in sizes[1:]] #隐藏层和输出层的每个神经元都有一个偏置。[[30,1],[10,1]]
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        for j in range(epochs): #每一代
            ...
            #当迭代到第29次时，将其结果保存下来。
            if j==2:
                np.savez('D:\\中国地震局第二检测中心\\机器学习\\progect_35\\out\\weight.npz',out_w=self.weights)
                np.savez('D:\\中国地震局第二检测中心\\机器学习\\progect_35\\out\\biases.npz',out_b=self.biases)
