...
class DataReader(object):
  ...
  def thread_main(self, n_threads=1, start=0):
    ...
    for i in index:
      ...
      if np.random.random() < 0.95:
        ...
        itp_list = []
        its_list = []
      #在这将数据保存成npz格式的数据，文件名称是npz数据原始文件名称，只是存放在sample文件夹下，画一下看看，然后处理后的数据也保存，画图看看
      np.savez('./sample/cut/{}'.format(self.data_list.iloc[i]['fname']),sample)
      #print (sample)
      
      
      sample = self.normalize(sample)
      np.savez('./sample/normali/{}'.format(self.data_list.iloc[i]['fname']),sample)
      print (sample.shape)
      sample = self.adjust_missingchannels(sample)
      np.savez('./sample/adjust_miss/{}'.format(self.data_list.iloc[i]['fname']),sample)  
  ...
  def start_threads(self, n_threads=8):
    print ('test start_threads')
    for i in range(n_threads):
      thread = threading.Thread(target=self.thread_main, args=(n_threads, i))
      #thread.daemon = True
      thread.start()
